  ...
  return (
    <div className="flex flex-col gap-8 sm:gap-10 py-6 px-4 max-w-5xl mx-auto z-10 relative">
      ...
      <div className="text-center flex flex-col items-center gap-3">
        ...
        <p className="text-sm text-slate-700 dark:text-gray-400 max-w-xl leading-relaxed">
          {t('about_sub')}
        </p>
      </div>
      ...
      <section className="glass-surface p-6 sm:p-8 rounded-2xl border flex flex-col md:flex-row items-center gap-6">
        <div className="w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0 bg-slate-100 dark:bg-black/40 p-3.5 rounded-2xl border border-black/10 dark:border-white/10 flex items-center justify-center shadow-xs">
        ...
        <div className="flex flex-col gap-2.5 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-1">
            <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-gray-200 text-[11px] font-semibold border border-black/10 dark:border-white/10">
              Version 2.5 Pro
            </span>
            <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-gray-200 text-[11px] font-semibold border border-black/10 dark:border-white/10">
              Live Audio Previews
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
        <div className="glass-surface p-6 rounded-2xl border flex flex-col justify-between gap-4">
          ...
          <div className="bg-slate-100 dark:bg-black/40 p-3 rounded-xl border border-black/5 dark:border-white/5 text-[11px] text-slate-600 dark:text-gray-400">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {historyMilestones.map((m, idx) => (
            <div key={idx} className="glass-surface p-5 rounded-2xl flex flex-col gap-2 border">
              ...
              <p className="text-xs text-slate-700 dark:text-gray-300 leading-relaxed">{m.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="glass-surface p-6 rounded-2xl flex flex-col gap-5 border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {controversies.map((c, i) => (
            <div key={i} className="bg-slate-100 dark:bg-black/40 p-4 rounded-xl border border-black/5 dark:border-white/5 flex flex-col gap-1.5">
              <h4 className="font-bold text-xs text-gray-900 dark:text-white">{c.title}</h4>
              <p className="text-[11px] text-slate-700 dark:text-gray-400 leading-relaxed">{c.detail}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {faqs.map((faq, idx) => (
            <div key={idx} className="glass-surface p-5 rounded-2xl flex flex-col gap-2 border">
              ...
              <p className="text-xs text-slate-700 dark:text-gray-300 leading-relaxed">{faq.a}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="glass-surface p-6 sm:p-8 rounded-2xl text-center flex flex-col items-center gap-3 border">
        ...
        <p className="text-xs text-slate-700 dark:text-gray-400 max-w-md">{t('about_portal_sub')}</p>
        <div className="flex flex-wrap items-center justify-center gap-3 mt-2">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="px-5 py-2.5 rounded-full bg-slate-100 dark:bg-white/10 border border-black/10 dark:border-white/10 text-gray-900 dark:text-white font-semibold text-xs hover:bg-black/10 dark:hover:bg-white/20 transition-colors flex items-center gap-1.5"
          >
