...
import { Search, Disc, Users, Image as ImageIcon } from 'lucide-react';
      ...
      {query.trim() !== '' ? (
        totalResults > 0 ? (
          ...
          <div className="glass-surface p-12 rounded-3xl text-center flex flex-col items-center gap-4">
            ...
            <h3 className="text-xl font-bold text-white">{t('search_empty_title')} "{query}"</h3>
            <p className="text-xs text-gray-400">{t('search_empty_sub')}</p>
          </div>
        )
      ) : (
        <div className="glass-surface p-12 rounded-3xl text-center flex flex-col items-center gap-4">
          <span className="text-5xl">✨</span>
          <h3 className="text-xl font-bold text-white">{t('search_start_title')}</h3>
          <p className="text-xs text-gray-400">{t('search_start_sub')}</p>
        </div>
      )}
    </div>
  );
